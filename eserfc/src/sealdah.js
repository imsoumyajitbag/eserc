...
async function fetchSealdahData() {
    const sealdahQuery = query(collection(db, "Rake"), where("trs", "in", ["NKG", "RHA", "SPR", "BT"]));
    const querySnapshot = await getDocs(sealdahQuery);
...
document.addEventListener('DOMContentLoaded', fetchSealdahData);
